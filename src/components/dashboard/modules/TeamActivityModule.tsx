...
const STATUS_CONFIG: Record<string, { color: string; bg: string; label: string }> = {
  hadir: { color: 'text-emerald-400', bg: 'bg-emerald-500/20', label: 'Hadir' },
  izin: { color: 'text-amber-400', bg: 'bg-amber-500/20', label: 'Izin' },
  sakit: { color: 'text-orange-400', bg: 'bg-orange-500/20', label: 'Sakit' },
  alpha: { color: 'text-red-400', bg: 'bg-red-500/20', label: 'Alpha' },
  lembur: { color: 'text-blue-400', bg: 'bg-blue-500/20', label: 'Lembur' },
};
...
export default function TeamActivityModule() {
  ...
  const loadData = useCallback(() => {
    setData(getData<TeamActivity>(KV_PREFIXES.teamActivity));
  }, []);
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-white">Aktivitas Tim</h1>
          <p className="text-white/40 text-sm mt-1">Pencatatan kehadiran dan aktivitas karyawan</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => exportToCSV(filtered.map(({ id, createdAt, updatedAt, ...rest }) => rest), 'aktivitas-tim')} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/[0.07] border border-white/[0.12] text-white/70 hover:bg-white/10 text-sm transition-all">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <GlassCard className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold text-white">{hadirCount}</p>
        ...
        <GlassCard className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold text-white">{lemburCount}</p>
              <p className="text-white/40 text-xs">Lembur</p>
            </div>
          </div>
        </GlassCard>
        <GlassCard className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold text-white">{izinCount}</p>
              <p className="text-white/40 text-xs">Izin</p>
            </div>
          </div>
        </GlassCard>
        <GlassCard className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold text-white">{alphaCount}</p>
              <p className="text-white/40 text-xs">Alpha</p>
            </div>
          </div>
        </GlassCard>
      </div>
      ...
      <GlassCard className="p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Cari karyawan atau divisi..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="w-full pl-9 pr-4 py-2.5 bg-white/[0.05] border border-white/[0.1] rounded-xl text-white text-sm placeholder:text-white/30 focus:border-cyan-500/40 focus:outline-none" />
          </div>
          <input type="date" value={filterDate} onChange={(e) => { setFilterDate(e.target.value); setPage(1); }} className="px-4 py-2.5 bg-white/[0.05] border border-white/[0.1] rounded-xl text-white text-sm focus:border-cyan-500/40 focus:outline-none" />
        </div>
      </GlassCard>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10">
                <th className="text-left px-4 py-3 text-white/40 font-medium">Karyawan</th>
                <th className="text-left px-4 py-3 text-white/40 font-medium">Divisi</th>
                <th className="text-left px-4 py-3 text-white/40 font-medium">Aktivitas</th>
                <th className="text-left px-4 py-3 text-white/40 font-medium">Status</th>
                <th className="text-left px-4 py-3 text-white/40 font-medium">Jam Masuk</th>
                <th className="text-left px-4 py-3 text-white/40 font-medium">Jam Keluar</th>
                ...
                paged.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="border-b border-white/5 hover:bg-white/[0.03] transition-colors">
                      <td className="px-4 py-3 text-white font-medium">{item.namaKaryawan}</td>
                      <td className="px-4 py-3 text-white/50">{item.divisi}</td>
                      <td className="px-4 py-3 text-white/60">{item.aktivitas}</td>
                      <td className="px-4 py-3">
                        <span className={cn('px-2 py-0.5 rounded-full text-xs font-medium', sc.bg, sc.color)}>{sc.label}</span>
                      </td>
                      <td className="px-4 py-3 text-white/50">{item.jamMasuk}</td>
                      <td className="px-4 py-3 text-white/50">{item.jamKeluar}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-[#0f0c29] border-white/10 backdrop-blur-xl max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-white">{editingItem ? 'Edit Aktivitas' : 'Tambah Aktivitas'}</DialogTitle>
            <DialogDescription className="text-white/40">{editingItem ? 'Perbarui data aktivitas' : 'Isi data aktivitas karyawan'}</DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 py-4">
            <div>
              <label className="text-white/50 text-xs mb-1 block">Nama Karyawan *</label>
              <input value={form.namaKaryawan} onChange={(e) => setForm({ ...form, namaKaryawan: e.target.value })} className="w-full px-3 py-2 bg-white/[0.05] border border-white/[0.1] rounded-xl text-white text-sm placeholder:text-white/30 focus:border-cyan-500/40 focus:outline-none" placeholder="Nama karyawan" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as TeamActivity['status'] })} className="w-full px-3 py-2 bg-white/[0.05] border border-white/[0.1] rounded-xl text-white text-sm focus:border-cyan-500/40 focus:outline-none appearance-none">
                  {Object.entries(STATUS_CONFIG).map(([k, v]) => <option key={k} value={k} className="bg-[#0f0c29]">{v.label}</option>)}
                </select>
              </div>
            </div>
            <div>
              <label className="text-white/50 text-xs mb-1 block">Aktivitas</label>
              <input value={form.aktivitas} onChange={(e) => setForm({ ...form, aktivitas: e.target.value })} className="w-full px-3 py-2 bg-white/[0.05] border border-white/[0.1] rounded-xl text-white text-sm placeholder:text-white/30 focus:border-cyan-500/40 focus:outline-none" placeholder="Deskripsi aktivitas" />
            </div>
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                <label className="text-white/50 text-xs mb-1 block">Jam Masuk</label>
                <input type="time" value={form.jamMasuk} onChange={(e) => setForm({ ...form, jamMasuk: e.target.value })} className="w-full px-3 py-2 bg-white/[0.05] border border-white/[0.1] rounded-xl text-white text-sm focus:border-cyan-500/40 focus:outline-none" />
              </div>
              <div>
                <label className="text-white/50 text-xs mb-1 block">Jam Keluar</label>
                <input type="time" value={form.jamKeluar} onChange={(e) => setForm({ ...form, jamKeluar: e.target.value })} className="w-full px-3 py-2 bg-white/[0.05] border border-white/[0.1] rounded-xl text-white text-sm focus:border-cyan-500/40 focus:outline-none" />
              </div>
            </div>
            <div>
      ...
      <Dialog open={!!deleteConfirm} onOpenChange={() => setDeleteConfirm(null)}>
        <DialogContent className="bg-[#0f0c29] border-white/10 backdrop-blur-xl max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-white">Hapus Aktivitas?</DialogTitle>
